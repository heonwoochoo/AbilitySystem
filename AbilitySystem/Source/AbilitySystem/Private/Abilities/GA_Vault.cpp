...
bool UGA_Vault::CommitCheck(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, OUT FGameplayTagContainer* OptionalRelevantTags)
{
	...
	AAbilitySystemCharacter* Character = GetCharacterFromActorInfo();
...
void UGA_Vault::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	UAG_MotionWarpingComponent* CharacterMotionWarpingComponent = Character ? Character->GetCharacterMotionWarpingComponent() : nullptr;
	if (CharacterMotionWarpingComponent)
	{
		CharacterMotionWarpingComponent->AddOrUpdateWarpTargetFromLocationAndRotation(TEXT("JumpToLocation"), JumpToLocation, Character->GetActorRotation());
		CharacterMotionWarpingComponent->AddOrUpdateWarpTargetFromLocationAndRotation(TEXT("JumpOverLocation"), JumpOverLocation, Character->GetActorRotation());
	}
...
void UGA_Vault::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
	...
	if (CharacterMotionWarpingComponent)
	{
		CharacterMotionWarpingComponent->RemoveWarpTarget(TEXT("JumpToLocation"));
		CharacterMotionWarpingComponent->RemoveWarpTarget(TEXT("JumpOverLocation"));
	}
